...
class Meta(Cog):
    ...
    async def show_bot_stats(self, ctx):
        embed = Embed(title='Bot stats', colour=ctx.author.colour,
                      thumbnail=self.bot.user.avatar_url, timestamp=datetime.utcnow())
        proc = Process()
        with proc.oneshot():
            ...
            mem_usuage = mem_total * (mem_of_total/100)
        fields = [
            ...
            ('Memory Usuage',
             f'{mem_usuage:.3f} / {mem_total:.0f} MiB ({mem_of_total*100:.0f}%)', True),
    ...
    async def shutdown(self, ctx):
        ...
        with open('./data/banlist.txt', 'w', encoding='utf-8') as f:
            f.writelines([f'{item}' for item in self.bot.banlist])
        db.commit()
